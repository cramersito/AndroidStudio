...
public class GameEngine {
    ...
    public void onGameEvent (GameEvent gameEvent) {
        ...
        if (gameEvent == GameEvent.Death){
            ScaffoldActivity a = (ScaffoldActivity) mainActivity;

            for (int i=0; i<numObjects; i++) {

                //Looks for sscore object to write score in preferences
                if (gameObjects.get(i) instanceof ScoreGameObject){

                    SharedPreferences sharedPref = mainActivity.getPreferences(Context.MODE_PRIVATE);
                    SharedPreferences.Editor editor = sharedPref.edit();
                    String score = ((ScoreGameObject) gameObjects.get(i)).getScore();
